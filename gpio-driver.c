...
static int gpio_sysfs_probe(struct platform_device *pdev)
{
  ...
  gpio_drv_priv_data.dev = devm_kzalloc(dev, sizeof(struct device*) * gpio_drv_priv_data.total_devices, GFP_KERNEL);
  ...
  for_each_available_child_of_node(parent, child) {
    ...
    dev_data->desc = devm_fwnode_get_gpiod_from_child(dev, "bcm2711", &child->fwnode, GPIOD_ASIS, dev_data->label);
